"""
Модуль для агрегации данных по всему магазину для РНП.
Суммирует метрики всех артикулов без группировки по категориям.
"""
import logging
import decimal
import psycopg2
...
def get_data_by_shop(date_range: Set[date], db_url: str) -> Dict[tuple, Dict[str, Any]]:
    """
    Извлекает агрегированные данные из БД по всему магазину.
    ...
    Returns:
        Словарь вида {('Магазин', date): {attr: value}}
    """
    logging.info(f"Запрос агрегированных данных по магазину для {len(date_range)} дат...")
    ...
    SELECT
        ...
        -- Агрегированные метрики по всему магазину
    ...
    GROUP BY COALESCE(cr.date_of_period, adv.date)
    ORDER BY "date";
    """
    ...
    try:
        ...
        logging.info(f"Выполняю SQL-запрос агрегации по магазину...")
        ...
        logging.info(f"✅ Из БД извлечено {len(db_data_map)} агрегированных записей по магазину.")
