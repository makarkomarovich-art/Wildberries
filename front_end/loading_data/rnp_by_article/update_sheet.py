...
try:
    from .aggregate_data import get_data_by_articles
except ImportError:
    # Прямой запуск - импортируем через абсолютный путь
    from front_end.loading_data.rnp_by_article.aggregate_data import get_data_by_articles
    ...
    for nm_id, article_info in validated_articles.items():
        for attr_name, row_num in article_info['attributes'].items():
            ...
            for date_obj, col_num in date_columns.items():
                ...
                # Увеличенный порог для учета округления Google Sheets (до 10 знаков после запятой)
                if not isinstance(gs_value, str) and not abs(float(gs_value) - float(db_value)) < 1e-6:
...
def main():
    ...
    db_data = get_data_by_articles(set(validated_articles.keys()), set(date_columns.keys()), SUPABASE_DB_URL)
    if db_data is None:
