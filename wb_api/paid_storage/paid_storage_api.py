#!/usr/bin/env python3
"""
API client for Seller Analytics Paid Storage report.

Endpoints:
- Create task: GET /api/v1/paid_storage?dateFrom=YYYY-MM-DD&dateTo=YYYY-MM-DD
- Download    : GET /api/v1/paid_storage/tasks/{task_id}/download

Behavior:
- Authorization header uses WB_API_TOKEN from api_keys.py
- Polling download every 2 seconds up to 30 attempts until report is ready
- When executed directly (if __name__ == "__main__"), saves raw JSONs into wb_api/paid_storage
...
CREATE_PATH = "/api/v1/paid_storage"
DOWNLOAD_PATH_TMPL = "/api/v1/paid_storage/tasks/{task_id}/download"
...
def download_report(
    ...
    *,
    ...
    """
    Try to download the paid storage report for given task.
    Returns parsed JSON (array) on success, otherwise raises.
    """
    token = token or api_keys.WB_API_TOKEN
    headers = {"Authorization": token}
    ...
    if not isinstance(data, list):
        raise RuntimeError("Download returned non-array JSON")
    return data
...
def fetch_report_with_polling(
    ...
    *,
    ...
    if save_dir is not None:
        ...
        out_path = save_dir / f"paid_storage_create_{ts}.json"
    ...
    while attempts < max_attempts:
        ...
        try:
            ...
            if save_dir is not None:
                save_dir.mkdir(parents=True, exist_ok=True)
                ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                out_path = save_dir / f"paid_storage_download_{ts}.json"
                with out_path.open("w", encoding="utf-8") as f:
...
def _default_period_last_7_days() -> Tuple[date, date]:
    today = date.today()
...
if __name__ == "__main__":
    """
    Standalone run: fetch last 7 days and save raw JSONs into wb_api/paid_storage.
    Intentionally keeps all saving logic inside this module (not in main_function).
    """
    try:
        begin, end = _default_period_last_7_days()
        save_dir = SCRIPT_DIR  # wb_api/paid_storage
        ...
        out_path = save_dir / f"paid_storage_{ts}.json"
        with out_path.open("w", encoding="utf-8") as f:
